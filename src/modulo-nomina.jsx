...
function TrabajadorModal({ trabajador, valoresIniciales, onSave, onClose, areasNomina, areasTNS, zonasNomina, tiposContrato, turnos, gruposTrabajo }) {
  ...
  // solo sabe manejar uno u otro).
  function setSalarioMinimoGarantizado(v) {
    setForm((f) => ({ ...f, salarioMinimoGarantizado: v, pagoPorDia: v ? false : f.pagoPorDia }));
  }
  function setPagoPorDia(v) {
    setForm((f) => ({ ...f, pagoPorDia: v, salarioMinimoGarantizado: v ? false : f.salarioMinimoGarantizado }));
  }
      ...
      {form.tipoNomina === "Destajo" && (
        <>
          <Field label="Salario mínimo garantizado">
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" id="salarioMinimoGarantizado" checked={!!form.salarioMinimoGarantizado} onChange={(e) => setSalarioMinimoGarantizado(e.target.checked)} />
              <label htmlFor="salarioMinimoGarantizado" style={{ fontSize: 12.5, color: C.ink, cursor: "pointer" }}>Pagarle siempre el salario mínimo (no por producción)</label>
            </div>
          </Field>
          <div style={{ fontSize: 11, color: C.slate, marginTop: -8, marginBottom: 8 }}>
            (2026-09-18, a pedido de Fredy) Con esto marcado, en su liquidación de Destajo ya NO se le paga lo que produjo -- se le paga fijo el salario mínimo de la quincena + su auxilio de transporte, sin importar cuánto haya producido. Su producción real se sigue guardando (para el histórico); si esa quincena produjo menos de lo que se le pagó fijo, la diferencia queda registrada sola como "ayuda" en el Ajuste de esa liquidación.
          </div>
          <Field label="Pagar por día">
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input type="checkbox" id="pagoPorDia" checked={!!form.pagoPorDia} onChange={(e) => setPagoPorDia(e.target.checked)} />
              <label htmlFor="pagoPorDia" style={{ fontSize: 12.5, color: C.ink, cursor: "pointer" }}>Pagarle por día trabajado (no por producción ni sueldo fijo)</label>
            </div>
          </Field>
          {form.pagoPorDia && (
            <Field label="Valor día"><FInput type="number" value={form.valorDia} onChange={set("valorDia")} placeholder="Ej: 45000" /></Field>
          )}
          <div style={{ fontSize: 11, color: C.slate, marginTop: -8, marginBottom: 8 }}>
            (2026-09-18, a pedido de Fredy) Con esto marcado, se le paga (días trabajados según el huellero de la quincena) × (Valor día), más las Horas Sueltas que le registres -- sin producción, sin auxilio de transporte y sin cesantías/prima/vacaciones en la quincena. El "Sueldo" y "Auxilio de Transporte" de arriba se dejan de todos modos, porque esos sí se usan (solo) para su Liquidación de Retiro. Es excluyente con "Salario mínimo garantizado".
          </div>
        </>
      )}
...
function NominaFiscalView({ trabajadores, faltas, ausencias, motivosDisponibles, onJustificarFalta, onLimpiarFaltaJustificada, diasTrabajados, liquidaciones, onGuardarTrabajador, onGuardarLiquidacion, lotesConCobros, onMarcarCobrosCobrados, turnos, areasNomina, horas, deduccionesTrabajador, causacionManual, horasExtras, bonificaciones, isAdmin, onAbrirQuincena, onEliminarLiquidacion }) {
  ...
  function calcular() {
    const filas = personas.map((t) => {
      ...
      // (2026-09-21, a pedido de Fredy) Bonificación puntual -- siempre en
      // efectivo (ver Financiera).
      const bonificacionPuntual = valorBonificacion(bonificaciones, t.id, periodoId);
      ...
      return { trabajador: t, calculo: { ...base, fechasFalta: faltasDetalle.map((f) => f.fecha), diasTrabajados: diasTrabajadosCount, horasDetalle, horasCant, totalHoras, totalHorasExtra, bonificacionPuntual, descuentoCobros, cobrosDetalle, deduccionesDetalle, descuentoDeducciones, netoAPagar: base.netoAPagar + totalHoras + totalHorasExtra + bonificacionPuntual - descuentoCobros - descuentoDeducciones } };
    });
    setResultados(filas);
    setGuardadoOk(false);
  }
  ...
  const totales = resultados ? resultados.reduce((s, r) => ({
    ...
    bonificacionPuntual: s.bonificacionPuntual + (r.calculo.bonificacionPuntual || 0),
  ...
  }), { neto: 0, descuentoCobros: 0, descuentoDeducciones: 0, totalHoras: 0, totalHorasExtra: 0, bonificacionPuntual: 0, epsTrabajador: 0, pensionTrabajador: 0, pensionEmpleador: 0, arlEmpleador: 0, cajaCompensacionEmpleador: 0, cesantias: 0, intereses: 0, prima: 0, vacaciones: 0, bonificacionCausacion: 0, ayudaCausacion: 0 }) : null;
  const busquedaNorm = normalizarNombreParaComparar(busqueda);
      ...
      {resultados && (
        <>
          <div style={{ display: "flex", gap: 14, marginBottom: 18, flexWrap: "wrap" }}>
            ...
            {totales.bonificacionPuntual > 0 && <KPI icon="🎯" label="Bonificación puntual" value={fmtMoney(totales.bonificacionPuntual)} color={C.green} bg={C.greenBg} />}
          ...
          <Tabla
            ...
            columnas={[
              ...
              { key: "bonificacionPuntual", label: "Bonificación", align: "right", render: (f) => (f.calculo.bonificacionPuntual || 0) > 0 ? <span style={{ color: C.green, fontWeight: 700 }}>{fmtMoney(f.calculo.bonificacionPuntual)}</span> : <span style={{ color: C.slate }}>—</span> },
...
function NominaFiscalDestajoView({ trabajadores, faltas, ausencias, motivosDisponibles, onJustificarFalta, onLimpiarFaltaJustificada, diasTrabajados, liquidaciones, onGuardarTrabajador, onGuardarLiquidacion, lotesConCobros, onMarcarCobrosCobrados, turnos, horas, deduccionesTrabajador, causacionManual, horasExtras, bonificaciones, isAdmin, onAbrirQuincena, onEliminarLiquidacion }) {
  ...
  const totales = resultados ? resultados.reduce((s, r) => ({
    ...
    bonificacionCausacion: s.bonificacionCausacion + (r.calculo.bonificacionCausacion || 0),
    ayudaCausacion: s.ayudaCausacion + (r.calculo.ayudaCausacion || 0),
  }), { neto: 0, descuentoCobros: 0, descuentoDeducciones: 0, totalHoras: 0, totalHorasExtra: 0, bonificacionPuntual: 0, cesantias: 0, intereses: 0, prima: 0, vacaciones: 0, bonificacionCausacion: 0, ayudaCausacion: 0 }) : null;
  const busquedaNorm = normalizarNombreParaComparar(busqueda);
  const resultadosFiltrados = resultados && busquedaNorm ? resultados.filter((f) => normalizarNombreParaComparar(f.trabajador.nombre).includes(busquedaNorm)) : resultados;
  async function exportarExcel() {
    const filas = resultadosFiltrados || [];
...
function exportReciboLiquidacionHTML({ tipoNomina, trabajador, liquidacion }) {
  ...
  const filasPago = esPrestacionServicios
    ...
    ? `
      ...
      <tr><td>Sueldo quincena</td><td style="text-align:right">${fmtMoney(liquidacion.sueldoQuincena)}</td></tr>
      <tr><td>Auxilio quincena</td><td style="text-align:right">${fmtMoney(liquidacion.auxilioQuincena)}</td></tr>`
    : liquidacion.pagoPorDia
    ? `
      <tr><td>Días trabajados (huellero)</td><td style="text-align:right">${liquidacion.diasPagados == null ? "—" : liquidacion.diasPagados}</td></tr>
      <tr><td>Valor día</td><td style="text-align:right">${fmtMoney(liquidacion.valorDia)}</td></tr>
      <tr><td>Subtotal pago por día</td><td style="text-align:right">${fmtMoney(liquidacion.pagoDias)}</td></tr>`
    : `
...
<div class="page">
  ...
  <div class="body">
    ...
    <table><tbody>
      <tr><td>Horas sueltas pagadas en la quincena (ya sumadas al neto)</td><td style="text-align:right;color:#2D9E6B">+${fmtMoney(liquidacion.totalHoras)}</td></tr>
    </tbody></table>` : ""}
...
function NominaPrestacionServicioView({ trabajadores, liquidaciones, onGuardarLiquidacion, lotesConCobros, onMarcarCobrosCobrados, deduccionesTrabajador, horasExtras, bonificaciones, isAdmin, onAbrirQuincena, onEliminarLiquidacion }) {
  ...
  function calcular() {
    const filas = personas.map((t) => {
      ...
      const totalHorasExtra = sumaHorasExtraTrabajador(horasExtras, t.id, inicio, fin);
      // (2026-09-21, a pedido de Fredy) Bonificación puntual -- aplica
      // tambien a Prestación de Servicios, siempre en efectivo.
      const bonificacionPuntual = valorBonificacion(bonificaciones, t.id, periodoId);
      return { trabajador: t, calculo: { ...base, descuentoCobros, cobrosDetalle, deduccionesDetalle, descuentoDeducciones, totalHorasExtra, bonificacionPuntual, netoAPagar: base.netoAPagar - descuentoCobros - descuentoDeducciones + totalHorasExtra + bonificacionPuntual } };
    });
    setResultados(filas);
    setGuardadoOk(false);
  }
  ...
  const totales = resultados ? resultados.reduce((s, r) => ({
    ...
    totalHorasExtra: s.totalHorasExtra + (r.calculo.totalHorasExtra || 0),
    bonificacionPuntual: s.bonificacionPuntual + (r.calculo.bonificacionPuntual || 0),
  }), { neto: 0, descuentoCobros: 0, descuentoDeducciones: 0, totalHorasExtra: 0, bonificacionPuntual: 0 }) : null;
  const busquedaNorm = normalizarNombreParaComparar(busqueda);
  const resultadosFiltrados = resultados && busquedaNorm ? resultados.filter((f) => normalizarNombreParaComparar(f.trabajador.nombre).includes(busquedaNorm)) : resultados;
  async function exportarExcel() {
    const filas = resultadosFiltrados || [];
...
function calcularLiquidacionDestajo(trabajador, netoProduccion, totalHoras = 0, diasTrabajadosQuincena = 0) {
  // (2026-09-18, a pedido de Fredy) "Pagar por día": no se liquida por
  // producción ni por un sueldo fijo -- se paga (días trabajados según el
  // huellero de la quincena) x (Valor día), más las Horas Sueltas que le
  // registren. Sin auxilio de transporte ni cesantías/prima/vacaciones en
  // la quincena (eso, cuando corresponda, se calcula aparte en su
  // Liquidación de Retiro a partir del Sueldo/Auxilio que tenga guardados
  // en su ficha -- ver calcularLiquidacionRetiro, que no cambia con esto).
  // Excluyente con "Salario mínimo garantizado" (la ficha ya no deja
  // marcar los dos a la vez).
  if (trabajador.pagoPorDia) {
    const valorDia = Number(trabajador.valorDia) || 0;
    const diasPagados = Number(diasTrabajadosQuincena) || 0;
    const totalHorasNum = Number(totalHoras) || 0;
    const pagoDias = valorDia * diasPagados;
    return {
      netoBase: pagoDias + totalHorasNum, salarioMinimoGarantizado: false, ayudaSalarioMinimo: 0,
      produccionReal: 0, totalHoras: totalHorasNum,
      sueldoFijoQuincena: 0, auxilioFijoQuincena: 0,
      pagoPorDia: true, valorDia, diasPagados, pagoDias,
      cesantiasPeriodo: 0, interesesPeriodo: 0, primaPeriodo: 0, vacacionesPeriodo: 0,
      saldoCesantiasInicio: Number(trabajador.cesantiasAcumuladas) || 0, saldoCesantiasFin: Number(trabajador.cesantiasAcumuladas) || 0,
    };
  }
  const sueldo = Number(trabajador.sueldo) || 0;
  const auxilio = Number(trabajador.auxilioTransporte) || 0;
  ...
  // calcularLiquidacionFiscal y calcularLiquidacionFiscalDestajo. Estas
  // provisiones (cesantias/prima/vacaciones) NO cambian con la casilla de
  // "Salario mínimo garantizado" de abajo -- siguen usando el campo
  // "Sueldo" que ya tenga la ficha del trabajador, tal como siempre.
  ...
  const primaPeriodo = baseConAuxilio * TASA_PRIMA_MENSUAL;
  const vacacionesPeriodo = sueldoQuincena * TASA_VACACIONES_MENSUAL;
  // (2026-09-18, a pedido de Fredy) "Salario mínimo garantizado": a quien
  // tenga esta casilla marcada en su ficha ya NO se le paga por producción
  // -- se le paga fijo el salario mínimo LEGAL de la quincena (SMMLV_2026,
  // no el campo "Sueldo" de su ficha) + su auxilio de transporte, sin
  // importar cuánto haya producido. Su producción real se sigue guardando
  // (produccionReal) solo para saber si "alcanzó la meta"; si produjo
  // MENOS de lo que se le pagó fijo, esa diferencia es "la ayuda" que tuvo
  // que poner la empresa para completarle el sueldo -- se calcula sola,
  // nadie la digita (ver NominaDestajoView/ResumenSemanalView, columna
  // Ajuste). Si produjo igual o más, la ayuda queda en $0 -- no se le paga
  // de más por eso, es sueldo fijo.
  const salarioMinimoGarantizado = !!trabajador.salarioMinimoGarantizado;
  // (2026-09-18, a pedido de Fredy) Las Horas Sueltas de la quincena ahora
  // se suman al pago de Destajo (antes no se tomaban en cuenta aca y por
  // eso el Neto a Pagar salia distinto al de Resumen Semanal / Cierre de
  // Quincena, que si las suma) -- "totalBruto" es produccion + horas, y
  // tambien es lo que cuenta para saber si un trabajador con salario
  // minimo garantizado alcanzo la meta o no.
  const totalBruto = netoProduccion + (Number(totalHoras) || 0);
  let netoBase = totalBruto;
  let ayudaSalarioMinimo = 0;
  let sueldoFijoQuincena = 0;
  let auxilioFijoQuincena = 0;
  if (salarioMinimoGarantizado) {
    sueldoFijoQuincena = SMMLV_2026 / 2;
    auxilioFijoQuincena = auxilioQuincena;
    const pagoFijo = sueldoFijoQuincena + auxilioFijoQuincena;
    ayudaSalarioMinimo = Math.max(0, pagoFijo - totalBruto);
    netoBase = pagoFijo;
  }
  return {
    netoBase, salarioMinimoGarantizado, ayudaSalarioMinimo, produccionReal: netoProduccion, totalHoras: Number(totalHoras) || 0,
    sueldoFijoQuincena, auxilioFijoQuincena,
    cesantiasPeriodo, interesesPeriodo, primaPeriodo, vacacionesPeriodo,
    saldoCesantiasInicio, saldoCesantiasFin: saldoCesantiasInicio + cesantiasPeriodo,
  };
}
function NominaDestajoView({ trabajadores, produccion, faltas, ausencias, motivosDisponibles, onJustificarFalta, onLimpiarFaltaJustificada, diasTrabajados, liquidaciones, onGuardarTrabajador, onGuardarLiquidacion, lotesConCobros, onMarcarCobrosCobrados, areasNomina, ajustesDestajo, onGuardarAjusteDestajo, puedeAjustarDestajo, horas, causacionManual, horasExtras, bonificaciones, isAdmin, onAbrirQuincena, onEliminarLiquidacion }) {
  ...
  const [guardadoOk, setGuardadoOk] = useState(false);
  const [detalleFaltas, setDetalleFaltas] = useState(null); // { trabajador, fechas }
  // (2026-09-18, a pedido de Fredy) "Ajuste de Nómina" -- solo administrador,
  // Yuleisi Virginia y María Fernanda Páez (puedeAjustarDestajo) pueden
  // agregarle a un trabajador (que NO tenga salario mínimo garantizado) un
  // valor + observación en texto libre para esa quincena puntual. Se
  // recalcula solo (ver `resultadosFinal` más abajo), sin tener que volver
  // a darle "Calcular".
  const [ajustando, setAjustando] = useState(null); // trabajador o null
  const [ajusteValorForm, setAjusteValorForm] = useState("");
  ...
  function calcular() {
    const filas = personas.map((t) => {
      ...
      const totalHorasExtra = sumaHorasExtraTrabajador(horasExtras, t.id, inicio, fin);
      // (2026-09-21, a pedido de Fredy) Bonificación puntual -- se suma
      // siempre, incluso con Salario mínimo garantizado.
      const bonificacionPuntual = valorBonificacion(bonificaciones, t.id, periodoId);
      const base = calcularLiquidacionDestajo(t, netoProduccion, totalHorasQuincena, diasTrabajadosCount);
      const cobrosDetalle = cobrosPendientesDeTrabajador(lotesConCobros, t.id, fin);
      const descuentoCobros = sumaCobrosPendientes(cobrosDetalle);
      return { trabajador: t, calculo: { ...base, causacionManual: tieneCausacionManual, diasInasistencia, fechasFalta: faltasDetalle.map((f) => f.fecha), diasTrabajados: diasTrabajadosCount, horasDetalle, horasCant, totalHorasExtra, bonificacionPuntual, descuentoCobros, cobrosDetalle, netoAntesDeAjuste: base.netoBase - descuentoCobros + totalHorasExtra + bonificacionPuntual } };
    });
    setResultados(filas);
    setGuardadoOk(false);
  }
  // (2026-09-18, a pedido de Fredy) Capa final que le monta a cada fila el
  // Ajuste manual vigente (o, si el trabajador tiene salario mínimo
  // garantizado, la "ayuda" ya calculada sola) -- separado de `calcular()`
  // para que cambiar un Ajuste en Firestore recalcule esta pantalla al
  // instante, sin tener que volver a darle "Calcular".
  const resultadosFinal = useMemo(() => {
    if (!resultados) return null;
    return resultados.map((r) => {
      if (r.calculo.salarioMinimoGarantizado) {
        return { ...r, calculo: { ...r.calculo, ajusteValor: 0, ajusteObservacion: "", netoAPagar: r.calculo.netoAntesDeAjuste } };
      }
      const ajusteDoc = (ajustesDestajo || []).find((a) => a.id === `${r.trabajador.id}__${periodoId}`);
      const ajusteValor = ajusteDoc ? Number(ajusteDoc.valor) || 0 : 0;
      const ajusteObservacion = ajusteDoc ? (ajusteDoc.observacion || "") : "";
      return { ...r, calculo: { ...r.calculo, ajusteValor, ajusteObservacion, netoAPagar: r.calculo.netoAntesDeAjuste + ajusteValor } };
    });
  }, [resultados, ajustesDestajo, periodoId]);
  function abrirAjuste(t) {
    const existente = (ajustesDestajo || []).find((a) => a.id === `${t.id}__${periodoId}`);
    setAjusteValorForm(existente ? String(existente.valor) : "");
    setAjusteObsForm(existente ? existente.observacion || "" : "");
    setAjustando(t);
  }
  async function guardarAjuste() {
    if (!ajustando) return;
    await onGuardarAjusteDestajo({
      trabajadorId: ajustando.id,
      trabajadorNombre: ajustando.nombre,
      periodoId,
      valor: Number(ajusteValorForm) || 0,
      observacion: ajusteObsForm,
    });
    setAjustando(null);
  }

  async function confirmarYGuardar() {
    if (!resultadosFinal) return;
    setGuardando(true);
    try {
      for (const { trabajador, calculo } of resultadosFinal) {
        await onGuardarLiquidacion({
          id: `${trabajador.id}__${periodoId}`,
          periodoId, trabajadorId: trabajador.id, nombre: trabajador.nombre,
          inicio, fin, ...calculo,
  ...
  const totales = resultadosFinal ? resultadosFinal.reduce((s, r) => ({
    neto: s.neto + r.calculo.netoAPagar,
    descuentoCobros: s.descuentoCobros + (r.calculo.descuentoCobros || 0),
    totalHoras: s.totalHoras + (r.calculo.totalHoras || 0),
    totalHorasExtra: s.totalHorasExtra + (r.calculo.totalHorasExtra || 0),
    ...
    prima: s.prima + r.calculo.primaPeriodo,
    vacaciones: s.vacaciones + r.calculo.vacacionesPeriodo,
  }), { neto: 0, descuentoCobros: 0, totalHoras: 0, totalHorasExtra: 0, bonificacionPuntual: 0, ajustes: 0, ayudaSalarioMinimo: 0, cesantias: 0, intereses: 0, prima: 0, vacaciones: 0 }) : null;
  const busquedaNorm = normalizarNombreParaComparar(busqueda);
  ...
  return (
    <div>
      ...
      {ajustando && (
        <Modal title={`Ajuste de Nómina — ${ajustando.nombre}`} onClose={() => setAjustando(null)} width={480}>
          <div style={{ fontSize: 12, color: C.slate, marginBottom: 16 }}>
            Se descuenta o se suma solo de la quincena {periodoId} de este trabajador. Se recalcula solo, sin tener que volver a darle "Calcular".
          </div>
          <Field label="Valor del ajuste (positivo para sumar, negativo para restar)">
            <FInput type="number" value={ajusteValorForm} onChange={setAjusteValorForm} placeholder="Ej: 50000 o -50000" />
          </Field>
          <Field label="Observación">
            <FInput value={ajusteObsForm} onChange={setAjusteObsForm} placeholder="Ej: Le faltó contar 200 unidades de la referencia X" />
          </Field>
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
            <Btn variant="secondary" onClick={() => setAjustando(null)}>Cancelar</Btn>
            <Btn onClick={guardarAjuste}>Guardar ajuste</Btn>
          </div>
        </Modal>
      )}
      <div style={{ fontSize: 12, color: C.slate, marginBottom: 16, maxWidth: 780 }}>
        ...
        <>
          ...
          <Tabla
            ...
            columnas={[
              ...
              { key: "produccionReal", label: "Producción real", align: "right", render: (f) => f.calculo.pagoPorDia ? (
                <span title={`${f.calculo.diasPagados} día(s) × ${fmtMoney(f.calculo.valorDia)}`}><strong>{fmtMoney(f.calculo.pagoDias)}</strong> <span style={{ color: C.slate, fontWeight: 400 }}>({f.calculo.diasPagados} × {fmtMoney(f.calculo.valorDia)})</span></span>
              ...
              { key: "ajuste", label: "Ajuste / Ayuda", align: "right", render: (f) => {
                if (f.calculo.salarioMinimoGarantizado) {
                  return f.calculo.ayudaSalarioMinimo > 0 ? (
                    <span style={{ color: C.red, fontWeight: 700 }} title="Lo que produjo no alcanzó el salario mínimo garantizado -- esto es lo que tuvo que poner la empresa de más">🆘 {fmtMoney(f.calculo.ayudaSalarioMinimo)}</span>
                  ) : (
                    <span style={{ color: C.green, fontWeight: 700 }} title="Con lo que produjo alcanzaba igual o más del salario mínimo">✓ Alcanzó la meta</span>
                  );
                }
                if (puedeAjustarDestajo) {
                  return (
                    <Btn small variant="secondary" onClick={() => abrirAjuste(f.trabajador)}>
                      {f.calculo.ajusteValor ? `🛠 ${f.calculo.ajusteValor > 0 ? "+" : ""}${fmtMoney(f.calculo.ajusteValor)}` : "🛠 Ajuste"}
                    </Btn>
                  );
                }
                return f.calculo.ajusteValor ? (
                  <span style={{ color: f.calculo.ajusteValor > 0 ? C.green : C.red, fontWeight: 700 }} title={f.calculo.ajusteObservacion}>{f.calculo.ajusteValor > 0 ? "+" : ""}{fmtMoney(f.calculo.ajusteValor)}</span>
                ) : <span style={{ color: C.slate }}>—</span>;
              } },
              { key: "netoAPagar", label: "Neto a pagar", align: "right", render: (f) => <strong>{fmtMoney(f.calculo.netoAPagar)}</strong> },
              { key: "cesantiasPeriodo", label: "Cesantías (prov.)", align: "right", render: (f) => fmtMoney(f.calculo.cesantiasPeriodo) },
...
// (2026-09-21, a pedido de Fredy) "Bonificación puntual" -- valor manual
...
function exportDesprendiblePagoHTML({
  trabajador, desde, hasta, label, produccionItems, horasItems, totalProduccion, totalHoras, totalGeneral,
  cobrosDetalle, descuentoCobros,
  salarioMinimoGarantizado, sueldoFijoQuincena, auxilioFijoQuincena,
  pagoPorDia, valorDia, diasPagados, pagoDiasQuincena,
  ajusteValor, ajusteObservacion,
}) {
  const fechaGen = new Date().toISOString().slice(0, 10);
  ...
  const filasHoras = (horasItems || [])
    ...
    .join("");
  // (2026-09-18, a pedido de Fredy) Sección de Deducciones -- mismo detalle
  // de cobros pendientes (motivo/valor/fecha) que ya se ve en Nómina, para
  // que quede claro por qué el Total a Pagar es menor a lo producido.
  const filasDeducciones = (cobrosDetalle || [])
    .map(
      (c, i) => `
    <tr style="background:${i % 2 === 0 ? "#F7F4F0" : "#fff"}">
      <td style="padding:8px 10px;color:#5A5A7A;font-size:12px">${c.fecha ? fmtFechaISO(c.fecha) : "—"}</td>
      <td style="padding:8px 10px;font-weight:600">${c.tipo || "Cobro"}</td>
      <td style="padding:8px 10px;text-align:right;font-weight:700;color:#E85D4A">-${fmtMoney(c.valor)}</td>
    </tr>`
    )
    .join("");
  // (2026-09-18, a pedido de Fredy) A quien tenga "Salario mínimo
  // garantizado" ya no se le paga por producción -- el desprendible le
  // muestra Sueldo + Auxilio de Transporte en vez de la tabla de
  // Producción/Horas.
  const seccionProduccionYHoras = salarioMinimoGarantizado
    ? `<div class="section-title">💰 Salario</div>
    <div class="info-row" style="grid-template-columns:1fr 1fr">
      <div class="info-card"><label>Sueldo (quincena)</label><span>${fmtMoney(sueldoFijoQuincena)}</span></div>
      <div class="info-card"><label>Auxilio de Transporte</label><span>${fmtMoney(auxilioFijoQuincena)}</span></div>
    </div>`
    : pagoPorDia
    ? `<div class="section-title">📅 Pago por Día</div>
    <div class="info-row" style="grid-template-columns:1fr 1fr 1fr">
      <div class="info-card"><label>Días trabajados</label><span>${fmtNum(diasPagados)}</span></div>
      <div class="info-card"><label>Valor día</label><span>${fmtMoney(valorDia)}</span></div>
      <div class="info-card"><label>Subtotal</label><span>${fmtMoney(pagoDiasQuincena)}</span></div>
    </div>
    <div class="section-title">🕐 Horas Sueltas</div>
    ${
      filasHoras
  ...
  const seccionDeducciones = `
    <div class="section-title">🔻 Deducciones</div>
    ${
      filasDeducciones
        ? `<table>
          <thead><tr><th>Fecha</th><th>Motivo</th><th style="text-align:right">Valor</th></tr></thead>
          <tbody>${filasDeducciones}</tbody>
        </table>`
        : `<div class="no-rows">Sin deducciones pendientes esta quincena.</div>`
    }`;
  const seccionAjuste = (!salarioMinimoGarantizado && ajusteValor)
    ? `<div class="section-title">🛠 Ajuste de Nómina</div>
    <div class="info-card" style="max-width:100%">
      <label>${ajusteObservacion || "Ajuste"}</label>
      <span>${ajusteValor > 0 ? "+" : ""}${fmtMoney(ajusteValor)}</span>
    </div>`
    : "";
  // Tarjetas de totales -- varían según si es salario mínimo garantizado,
  // si tuvo deducciones y si tuvo un ajuste manual, para no mostrar
  // tarjetas vacías/en $0 sin sentido.
  const cards = [];
  if (salarioMinimoGarantizado) {
    cards.push({ label: "Sueldo + Auxilio", value: fmtMoney((sueldoFijoQuincena || 0) + (auxilioFijoQuincena || 0)), bg: "#EBF1F7", color: "#3D6B9E" });
  } else if (pagoPorDia) {
    cards.push({ label: `Pago por día (${fmtNum(diasPagados)} × ${fmtMoney(valorDia)})`, value: fmtMoney(pagoDiasQuincena), bg: "#EBF1F7", color: "#3D6B9E" });
    cards.push({ label: "Total Horas", value: fmtMoney(totalHoras), bg: "#F3EEF9", color: "#7B5EA7" });
  } else {
    cards.push({ label: "Total Producción", value: fmtMoney(totalProduccion), bg: "#EBF1F7", color: "#3D6B9E" });
    cards.push({ label: "Total Horas", value: fmtMoney(totalHoras), bg: "#F3EEF9", color: "#7B5EA7" });
  }
  if (descuentoCobros > 0) cards.push({ label: "Deducciones", value: `-${fmtMoney(descuentoCobros)}`, bg: "#FDF0EE", color: "#E85D4A" });
  if (!salarioMinimoGarantizado && ajusteValor) cards.push({ label: "Ajuste", value: `${ajusteValor > 0 ? "+" : ""}${fmtMoney(ajusteValor)}`, bg: "#FDF5E6", color: "#C47C1A" });
  cards.push({ label: "Total a Pagar", value: fmtMoney(totalGeneral), bg: "#EBF7F2", color: "#2D9E6B" });
  const totalesHTML = cards.map((c) => `<div class="total-card" style="background:${c.bg};color:${c.color}"><label>${c.label}</label><div class="val">${c.value}</div></div>`).join("");
  const html = `<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8"/>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .totales{margin-top:22px;display:grid;grid-template-columns:repeat(${cards.length},1fr);gap:12px}
  .total-card{border-radius:10px;padding:14px 16px;text-align:center}
  .total-card label{display:block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;opacity:0.85}
  .total-card .val{font-size:19px;font-weight:900}
  .firma{margin-top:44px;display:grid;grid-template-columns:1fr 1fr;gap:40px}
...
function ResumenSemanalView({ trabajadores, produccion, horas, isAdmin, areasNomina, puedeCerrarQuincena, cierres, onCerrar, onReabrir, lotesConCobros, ajustesDestajo, causacionManual, diasTrabajados, faltas, ausencias, turnos, deduccionesTrabajador, horasExtras, bonificaciones }) {
  ...
  // Ajuste de Nómina de cada trabajador con lo que aparece acá.
  ...
  function calcularPorTipo(tipo) {
    ...
    trabajadoresTipo.forEach((t) => {
      // (2026-09-18, a pedido de Fredy) Días trabajados (huellero) de la
      // quincena -- se necesita para "Pagar por día", igual que ya hace
      // Nómina Destajo.
      const nombreNorm = normalizarNombreHuellero(t.nombre);
    ...
    return [...mapa.values()]
      .map((g) => {
        ...
        if (tipoEsFiscal) {
          ...
          const pagoFijo = sueldoFijoQuincena + auxilioFijoQuincena;
          ayudaSalarioMinimo = Math.max(0, pagoFijo - totalBruto);
          // (2026-09-18, a pedido de Fredy) Cuando el trabajador SI supera
          // el minimo garantizado con su produccion + horas, igual se le
          // paga fijo el minimo (no se le paga de mas por lo que produjo
          // de mas) -- esto deja visible cuanto genero de mas sobre lo que
          // se le pago fijo. Es solo informativo: no se le paga ni se le
          // descuenta nada por esto.
          excedenteSobreMinimo = Math.max(0, totalBruto - pagoFijo);
          netoAntesDeAjuste = pagoFijo - descuentoCobros;
        } else {
          if (pagoPorDia) {
  ...
  const totalBonificacionPuntualGeneral = porTrabajador.reduce((s, g) => s + (g.bonificacionPuntual || 0), 0);
  ...
  function descargarDesprendible(g) {
    ...
    exportDesprendiblePagoHTML({
      ...
      cobrosDetalle: g.cobrosDetalle,
      descuentoCobros: g.descuentoCobros,
      salarioMinimoGarantizado: g.salarioMinimoGarantizado,
      sueldoFijoQuincena: g.sueldoFijoQuincena,
      auxilioFijoQuincena: g.auxilioFijoQuincena,
      pagoPorDia: g.pagoPorDia,
      valorDia: g.valorDia,
      diasPagados: g.diasTrabajadosCount,
      pagoDiasQuincena: g.pagoDiasQuincena,
      ajusteValor: g.ajusteValor,
      ajusteObservacion: g.ajusteObservacion,
...
export default function ModuloNomina({ currentUser, onVolver, onLogout, soloNovedades, puedeEditarCatalogos, puedeVerAnomaliasHuellero, puedeVerDuplicadosHuellero, puedeCerrarQuincena, puedeAgregarCobrosManual, puedeAjustarDestajo, puedeEditarHorasExtra }) {
  ...
  // (2026-09-21, a pedido de Fredy) "Bonificación puntual" -- estado aparte
  // de Horas Extras. Ver RegistrarBonificacionView.
  ...
  // (2026-09-18, a pedido de Fredy) "Ajuste de Nómina" de Destajo -- un
  // ajuste (valor + observación) por trabajador por quincena, editable
  // solo por quien tenga "nomina_ajustar_destajo" (administrador, Yuleisi
  // Virginia, María Fernanda Páez). Aplica solo a Destajo y solo a
  ...
  useEffect(() => {
    const unsubs = [
      ...
      onSnapshot(collection(db, "nomina_bonificaciones"), (snap) => setBonificaciones(snap.docs.map((d) => ({ ...d.data(), id: d.id })))),
  ...
  const NAV = areaLider
    ...
    : [
        ...
        { group: "Novedades", icon: "📣", items: [
            ...
            { id: "bonificaciones", icon: "🎯", label: "Bonificación" },
          ] },
  ...
  // (2026-09-21, a pedido de Fredy) "Bonificación puntual" -- aparte de
  // Horas Extras. Ver RegistrarBonificacionView.
  async function guardarBonificacion(b) { await fsSave("nomina_bonificaciones", b.id, b); }
  async function borrarBonificacion(id) { await fsDelete("nomina_bonificaciones", id); }
  ...
  // (2026-09-18, a pedido de Fredy) Guarda/corrige el Ajuste de Nómina de
  // un trabajador de Destajo para una quincena puntual -- id determinístico
  // (trabajador+periodo) para que volver a guardar sobre el mismo corrija
  // el valor/observación anterior en vez de duplicarlo.
  async function guardarAjusteDestajo({ trabajadorId, trabajadorNombre, periodoId, valor, observacion }) {
    await fsSave("nomina_ajustes_destajo", `${trabajadorId}__${periodoId}`, {
      trabajadorId,
