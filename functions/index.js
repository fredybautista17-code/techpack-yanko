 ...
 *
 * 7. `avisarVencidos` (programada, corre sola una vez al día a las 8am hora
 *    Bogotá): revisa TODOS los prototipos y las referencias dentro de cada
 *    cápsula, calcula si están "vencidos" con la MISMA regla que usa la app
 *    en pantalla, y manda un correo a la diseñadora asignada + a Dayana,
 *    Karen y Yuliana (encargada de colecciones, aux. de colecciones y
 *    directora creativa). Se manda UNA sola vez por cada vez que un ítem cae
 *    en "vencido" (ver `vencidoAvisadoEtapa` en cada documento).
 *
...
exports.adminCrearUsuario = onCall(
  ...
  async (request) => {
    ...
    const { name, username, password, role, isAdmin, clienteAsociado, email } = request.data || {};
    ...
    const authEmail = `${usernameNorm}@techpack-yanko.local`;
    let userRecord;
    try {
      userRecord = await admin.auth().createUser({ email: authEmail, password: String(password), displayName: nombreLimpio });
    } catch (err) {
      if (err.code === "auth/email-already-exists") {
    ...
    await docRef.set({
      ...
      // Correo (opcional): usado por avisarVencidos para saber a dónde
      // mandar los avisos de prototipos/cápsulas vencidos de este usuario.
