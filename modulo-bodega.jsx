...
const ESTILO_HEADER = { font: { bold: true, color: { rgb: "FFFFFF" } }, fill: { patternType: "solid", fgColor: { rgb: "1F4E78" } }, alignment: { horizontal: "center", vertical: "center", wrapText: true }, border: TODOS_LOS_BORDES };
const ESTILO_VALOR = { font: { bold: true }, fill: { patternType: "solid", fgColor: { rgb: "E2EFDA" } }, alignment: { horizontal: "center", vertical: "center" }, border: TODOS_LOS_BORDES };
const ESTILO_DATO = { fill: { patternType: "solid", fgColor: { rgb: "D9E6F5" } }, border: TODOS_LOS_BORDES, alignment: { vertical: "center" } };
...
async function exportarDespachoExcel(despacho) {
  ...
  const COL_HEADERS = ["REF", "CANTIDAD", "N° TRASLADO", "N° DE CORTE", "N° DE BULTO COMO VIENE MARCADOS", "DESCRIPCION", "MARCA", "SEGMENTO", "PRECIO", "DCTO", "TOTAL DCTTO", "TOTAL", ...tallas];

  const encabezado = [
    [null, null, null, null, null, null, null, null, ...tallas.map(() => null)],
  ...
  const aoa = [...encabezado, ...filas, [], filaTotales];
  ...
  const FILA_TOTALES = FILA_DATOS_INICIO + lineas.length + 1;
  const N_COLS = 12 + tallas.length;
  const COLS_MONEDA = [9, 10, 11, 12];

  [1, 3, 6].forEach((c) => setStyle(FILA_BLOQUE, c, ESTILO_HEADER));
  [2, 4, 7].forEach((c) => setStyle(FILA_BLOQUE, c, ESTILO_VALOR));
  for (let c = 1; c <= N_COLS; c++) setStyle(FILA_HEADER, c, ESTILO_HEADER);
  lineas.forEach((l, idx) => {
    const r = FILA_DATOS_INICIO + idx;
    for (let c = 1; c <= N_COLS; c++) setStyle(r, c, ESTILO_DATO, COLS_MONEDA.includes(c) ? FORMATO_MONEDA : undefined);
  });
  [[1, "TOTAL UND"], [2, totalUnd], [4, "TOTAL BTS"], [5, undefined], [11, "TOTAL"], [12, totalGeneral]].forEach(([c]) => {
    setStyle(FILA_TOTALES, c, ESTILO_TOTAL, c === 12 ? FORMATO_MONEDA : undefined);
  });

  ws["!cols"] = [{ wch: 3 }, { wch: 10 }, { wch: 9 }, { wch: 12 }, { wch: 11 }, { wch: 20 }, { wch: 28 }, { wch: 10 }, { wch: 10 }, { wch: 11 }, { wch: 9 }, { wch: 13 }, { wch: 13 }, ...tallas.map(() => ({ wch: 16 }))];
